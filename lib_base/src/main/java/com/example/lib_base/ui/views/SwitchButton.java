...
import com.example.lib_base.common.utils.ColorUtils;
...
public class SwitchButton extends CompoundButton {
    ...
    private void init(AttributeSet attrs) {
        ...
        mProcessAnimator = ObjectAnimator.ofFloat(this, "process", 0, 0)
                .setDuration(SwitchButton.DEFAULT_ANIMATION_DURATION);
        ...
        float margin = density * SwitchButton.DEFAULT_THUMB_MARGIN_DP;
        float marginLeft = 0;
        float marginRight = 0;
        float marginTop = 0;
        float marginBottom = 0;
        float thumbWidth = density * SwitchButton.DEFAULT_THUMB_SIZE_DP;
        float thumbHeight = density * SwitchButton.DEFAULT_THUMB_SIZE_DP;
        float thumbRadius = density * SwitchButton.DEFAULT_THUMB_SIZE_DP / 2;
        float backRadius = thumbRadius;
        Drawable backDrawable = null;
        ColorStateList backColor = null;
        float backMeasureRatio = SwitchButton.DEFAULT_BACK_MEASURE_RATIO;
        int animationDuration = SwitchButton.DEFAULT_ANIMATION_DURATION;
        boolean fadeBack = true;
        int tintColor = 0;
        String textOn = null;
        String textOff = null;
        float textMarginH = density * SwitchButton.DEFAULT_TEXT_MARGIN_DP;
        ...
        if (ta != null) {
            ...
            backRadius = ta.getDimension(R.styleable.SwitchButton_kswBackRadius,
                    thumbRadius + density * 2f);
            ...
            backMeasureRatio = ta
                    .getFloat(R.styleable.SwitchButton_kswBackMeasureRatio, backMeasureRatio);
            animationDuration = ta
                    .getInteger(R.styleable.SwitchButton_kswAnimationDuration, animationDuration);
            ...
            autoAdjustTextPosition = ta
                    .getBoolean(R.styleable.SwitchButton_kswAutoAdjustTextPosition,
                            autoAdjustTextPosition);
            ta.recycle();
        }
        ...
        if (mTintColor == 0) {
            ...
            boolean found = getContext().getTheme()
                    .resolveAttribute(R.color.colorAccent, typedValue, true);
            if (found) {
                mTintColor = typedValue.data;
            } else {
                mTintColor = SwitchButton.DEFAULT_TINT_COLOR;
            }
        }
        ...
        if (!mIsBackUseDrawable && mBackColor == null) {
            ...
            mNextBackColor = mBackColor.getColorForState(SwitchButton.CHECKED_PRESSED_STATE, mCurrBackColor);
        }
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        int minHeight = ceil(
                Math.max(mThumbSizeF.y, mThumbSizeF.y + mThumbMargin.top + mThumbMargin.right));
    ...
    private void setup() {
        ...
        if (mOnLayout != null && mOffLayout != null) {
            if (mThumbMargin.top + mThumbMargin.bottom > 0) {
                ...
                float addition =
                        (getMeasuredHeight() - getPaddingBottom() - getPaddingTop() - mThumbSizeF.y
        ...
        mBackRectF.set(backLeft + textDiffWidth,
                ...
                backLeft + mThumbMargin.left + Math
                        .max(mThumbSizeF.x * mBackMeasureRatio, mThumbSizeF.x + mTextWidth)
        ...
        if (mOnLayout != null) {
            float marginOnX = mBackRectF.left +
                    (mBackRectF.width() - mThumbRectF.width() - mThumbMargin.right - mOnLayout
                            .getWidth()) / 2 + (mThumbMargin.left < 0 ? mThumbMargin.left * -0.5f
                    : 0);
            ...
            mTextOnRectF.set(marginOnX, marginOnY, marginOnX + mOnLayout.getWidth(),
                    marginOnY + mOnLayout.getHeight());
        }

        if (mOffLayout != null) {
            float marginOffX = mBackRectF.right -
                    (mBackRectF.width() - mThumbRectF.width() - mThumbMargin.left - mOffLayout
                            .getWidth()) / 2 - mOffLayout.getWidth() + (mThumbMargin.right < 0 ?
                    mThumbMargin.right * 0.5f : 0);
            if (!mIsBackUseDrawable && mAutoAdjustTextPosition) {
    ...
    protected void drawableStateChanged() {
        ...
        int[] nextState = isChecked() ? SwitchButton.UNCHECKED_PRESSED_STATE
                : SwitchButton.CHECKED_PRESSED_STATE;
        ColorStateList textColors = getTextColors();
        if (textColors != null) {
            int defaultTextColor = textColors.getDefaultColor();
            mOnTextColor = textColors.getColorForState(SwitchButton.CHECKED_PRESSED_STATE, defaultTextColor);
            mOffTextColor = textColors.getColorForState(SwitchButton.UNCHECKED_PRESSED_STATE, defaultTextColor);
        }
        if (!mIsBackUseDrawable && mBackColor != null) {
    ...
    public void setThumbDrawableRes(int thumbDrawableRes) {
        ...
        Drawable drawable = getContext().getResources().getDrawable(thumbDrawableRes);
    ...
    public void setBackDrawableRes(int backDrawableRes) {
        ...
        Drawable drawable = getContext().getResources().getDrawable(backDrawableRes);
    ...
    public void setThumbColorRes(int thumbColorRes) {
        ...
        ColorStateList colorStateList = getContext().getResources()
                .getColorStateList(thumbColorRes);
    ...
    public void setThumbSize(PointF size) {
        if (size == null) {
            float defaultSize = getResources().getDisplayMetrics().density * SwitchButton.DEFAULT_THUMB_SIZE_DP;
